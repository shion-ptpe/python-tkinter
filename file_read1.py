...
class File_Selection(tk.Frame):
    ...
    def Make_button(self):
        file_button = tk.Button(self, text="ファイルを追加する",relief=tk.RAISED, cursor="hand2",font=("normal","30"))
        file_button.pack(side=tk.BOTTOM, anchor=tk.S)
    ...
    def content_button(event):
        ...
        la_name.pack(side=tk.TOP)
        la_content.pack(side=tk.TOP)

class Make_Scrollbar_config(tk.Frame):
    def __init__(self, root):   # ファイル選択のボタンを作る
        super().__init__(root,background="#ededed")  # super()でtk.Frameが呼ばれる(フレームが作られる)
        self.pack()
    ...
    def Make_Scrollbar_lists(self):
        ...
        ybar.grid(row=0, column=1, sticky=tk.N + tk.S)  # スクロールバーを上下いっぱいに引き伸ばす
    ...
    def Make_Scrollbar_content(self):
        ...
        canvas2=tk.Canvas(self, width=500, height=500,scrollregion=(0,0,3000,5000))  # scrollregioはスクロール時の描画
...
if __name__ == '__main__':
    ...
    frame2=Make_Scrollbar_config(root)
    frame2.Make_Scrollbar_lists()
